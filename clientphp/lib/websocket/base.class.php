...
class Base
{
	...
	public $socket, $is_connected = false;
	protected $is_closing = false, $last_opcode = null,
...
// And close the socket.
	...
	/*
	  ...
	  $data_l = strlen($this->buffer);

	  if ($data_l) {
	  $buffbefore = $this->buffer;





	  $cut = substr($this->buffer, 0, $length);
	  $this->buffer = substr($this->buffer, strlen($cut), $data_l);

	  //print_r(['cut'=>$cut, 'cut_length'=>strlen($cut), 'length'=>$length, 'remains'=>$this->buffer]);
	  //exit;
	...
	public function _encodeData($action, $data)
	{
		if (empty($action)) {
			return false;
		}
	...
	function writeData($action, $data = false)
	{
		$data = $this->_encodeData($action, $data);
