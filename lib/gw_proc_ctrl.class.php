...
class GW_Proc_Ctrl
{
	...
	{
		$list = shell_exec($cmd = "ps -p $pid -o cmd= 2>&1");
		//echo("rodou isrunning autputa ($cmd): $list\n");
		return strpos($list, $procname) !== false;
	}
	/*
	 * returns all if pid not specified
	 */

	static function getRunningProcessesInfo($pid = false)
	{
		//http://www.linux.ie/newusers/beginners-linux-guide/ps.php
		$out = shell_exec('ps ' . ($pid ? "-p $pid" : '-x') . ' -o "%p - %a" 2>/dev/null');
