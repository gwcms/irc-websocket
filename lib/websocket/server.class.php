...
class Server extends Socket
{
	...
	public function __construct($host = 'localhost', $port = 8000, $ssl = false, $controller = false, $conf = [])
	{

		$this->controller = $controller;
		$this->conf = $conf;

		parent::__construct($host, $port, $ssl, $conf);
	...
	public function run()
	{
		...
		$totalconnections= count($changed_sockets);
		...
		//if($changed_sockets)
		//	$this->log(count($changed_sockets)." / $totalconnections");
		...
		foreach ($changed_sockets as $socket) {
			if ($socket == $this->master) {
				...
				if ($bytes === 0) {
					$client->onDisconnect(3);
					continue;
				} elseif ($data === false) {
					$this->removeClientOnError($client, 1550);
					continue;
				} elseif ($client->waitingForData === false && $this->_checkRequestLimit($client->getClientId()) === false) {
					$client->onDisconnect(4);
				} else {
					$dataunprocessed = '';



					$client->onData($data, $dataunprocessed);

					while ($dataunprocessed) {
						echo "UNPROCESSED DATA: ".strlen($dataunprocessed)."\n";
	...
	public function registerApplication($key, $application)
	{
		...
		$this->applications[$key]->controller = $this->controller;
	}
	...
	public function log($message, $type = 'info')
	{
		...
		$this->controller->msg(' [' . ($type ? $type : 'error') . '] ' . $message);
	}
	...
	public function removeClientOnClose($client)
	{
		...
		$this->log("REMOVE resource int". (int) $resource);
		
		
		unset($this->clients[(int) $resource]);
		
		$this->log("REMOVE resource int". (int) $resource." count clients: ".count($this->clients));
		
		$index = array_search($resource, $this->allsockets);
		
		if($index!==false){
			$this->log("REMOVING SOCKET $index");
	 ...
	 */
	public function removeClientOnError($client, $errcode=1111)
	{  // remove reference in clients app:
		if ($client->getClientApplication() !== false) {
			$client->getClientApplication()->onDisconnect($client, $errcode);
		}
