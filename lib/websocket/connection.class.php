...
class Connection
{
	...
	public function __construct($server, $socket)
	{
		...
		$this->log('Connected ' . $this->connectionId);
	}
	...
	private function handshake($data)
	{
		...
		if (!$this->application) {
			...
			stream_socket_shutdown($this->socket, STREAM_SHUT_RDWR);
			$this->server->removeClientOnError($this, 1551);
			return false;
		}
		...
		if (!isset($headers['Sec-WebSocket-Version']) || $headers['Sec-WebSocket-Version'] < 6) {
			$this->log('Unsupported websocket version: ' . json_encode($headers) . ' versoin: ' . $headers['Sec-WebSocket-Version']);
			$this->sendHttpResponse(501);
			stream_socket_shutdown($this->socket, STREAM_SHUT_RDWR);
			$this->server->removeClientOnError($this, 1552);
			return false;
		}
		
		if(isset($headers['authorization']))
		{
			$this->authorization = $this->parseAuthorisation(explode(' ', $headers['authorization']));
		}
		...
		if ($this->server->getCheckOrigin() === true) {
			...
			if (empty($origin)) {
				...
				$this->sendHttpResponse(401);
				stream_socket_shutdown($this->socket, STREAM_SHUT_RDWR);
				$this->server->removeClientOnError($this, 1553);
				return false;
			}
			...
			if ($this->server->checkOrigin($origin) === false) {
				...
				$this->sendHttpResponse(401);
				stream_socket_shutdown($this->socket, STREAM_SHUT_RDWR);
				$this->server->removeClientOnError($this, 1554);
				return false;
			}
		}
	...
	private function parseAuthorisation($headerData)
	{
		if(count($headerData) > 1 && strtolower($headerData[0]) == 'basic')
		{
			return base64_decode($headerData[1]);
		}
	}
	...
	public function onData($data, &$notprocessed = false)
	{
		if ($this->handshaked) {
			return $this->handle($data, $notprocessed);
		} else {
			$this->handshake($data);
		}
	}

	private function handle($data, &$notprocessed = false)
	{
		...
		$decodedData = $this->hybi10Decode($data, $notprocessed);
		...
		if (!isset($decodedData['type'])) {
			$this->sendHttpResponse(401);
			stream_socket_shutdown($this->socket, STREAM_SHUT_RDWR);
			$this->server->removeClientOnError($this, 1555);
			return false;
		}
	...
	public function send($payload, $type = 'text', $masked = false)
	{
		...
		if (!$this->server->writeBuffer($this->socket, $encodedData)) {
			$this->server->removeClientOnError($this, 1556);
			return false;
		}
		return true;
	}
	...
	public function close($statusCode = 1000)
	{
		...
		if ($this->application) {
			$this->application->onDisconnect($this, 15);
		}
		stream_socket_shutdown($this->socket, STREAM_SHUT_RDWR);
	...
	public function onDisconnect($code)
	{
		...
		//$this->log("Conection close code $code");
	...
	private function hybi10Decode($data, &$notprocessed = false)
	{
		$payloadLength = '';
		$mask = '';
		$unmaskedPayload = '';
		...
		if (strlen($data) > $dataLength) {
			$notprocessed = substr($data, $dataLength);
		}
