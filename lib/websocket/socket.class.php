...
class Socket
{
	...
	public function __construct($host = 'localhost', $port = 8000, $ssl = false, $conf = [])
	{
	...
	private function applySSLContext()
	{
		...
		/*
		  $pem_passphrase = 'gw_wss';

		  // Generate PEM file
		  if (!file_exists($pem_file)) {
		  $this->log("file $pem_file not exists! Generating cert file");
		  ...
		  "commonName" => "gw.lt services",
		  "emailAddress" => "admin@gw.lt"
		  );
		...
		$ssl_opt = $this->conf['ssl_options'];

		foreach ($ssl_opt as $key => $val) {
			stream_context_set_option($this->context, 'ssl', $key, $val);
		}
		/*
